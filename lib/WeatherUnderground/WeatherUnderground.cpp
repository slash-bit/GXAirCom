...
bool WeatherUnderground::getData(String ID, String KEY, wData *data)
{                 // get Data from WU with Station-ID and API-Key
  ...
  if (doc["observations"][0]["lat"])
    data->lat = doc["observations"][0]["lat"].as<float>();
  if (doc["observations"][0]["lon"])
    data->lon = doc["observations"][0]["lon"].as<float>();
  if (doc["observations"][0]["metric"]["elev"])
    data->height = doc["observations"][0]["metric"]["elev"].as<float>();
  ...
  if (doc["observations"][0]["humidity"])
    data->humidity = doc["observations"][0]["humidity"].as<float>();
...
bool WeatherUnderground::sendData(String ID, String KEY, wData *data)
{ // send Data to WU with Station-ID and Station-Key
  bool bRet = true;
  if (xMutex == NULL)
  { // create new Mutex
    xMutex = new SemaphoreHandle_t();
