...
void FanetLora::clearNeighboursWeather(uint32_t tAct)
{
  ...
  { // check only every 5 seconds
    ...
    for (int i = 0; i < MAXNEIGHBOURS; i++)
    {
      if (neighbours[i].devId)
      {
        if ((tCheck - neighbours[i].tLastMsg) >= NEIGHBOURSLIFETIME)
        { // if we get no msg in 4min --> del neighbour
    ...
    for (int i = 0; i < MAXWEATHERDATAS; i++)
    {
      if (weatherDatas[i].devId)
      {
        if ((tCheck - weatherDatas[i].tLastMsg) >= NEIGHBOURSLIFETIME)
        { // if we get no msg in 4min --> del neighbour
          // log_i("clear slot %i devId %s",i,getDevId(neighbours[i].devId).c_str());
...
int8_t FanetLora::getWeatherinfo(uint8_t *buffer, uint16_t length)
{
  ...
  if (header & 0x01)
    expectedpacketLength += 1; // extended Header expected
  if (header & 0x02)
    expectedpacketLength += 1; // State of Charge  (+1byte
  if (header & 0x08)
    expectedpacketLength += 2; // Barometric pressure normailized (+2byte
  if (header & 0x10)
    expectedpacketLength += 1; // Humidity (+1byte:
  if (header & 0x20)
    expectedpacketLength += 3; // Wind (+3byte
  if (header & 0x40)
    expectedpacketLength += 1; // Temperature (+1byte
  ...
  index++;
  if (header & 0x01)
    index++; // additional header direct after first byte
  // integer values /
...
void FanetLora::handle_frame(Frame *frm)
{
  ...
  */
  ...
  else if (frm->type == 7)
  {
    ...
    actTrackingData.timestamp = frm->timeStamp; // copy timestamp
    if (frm->altitude > 0)
      actTrackingData.altitude = frm->altitude;
...
{
  if (aircraftType >= 0x80)
    return aircraftType & 0x7F; // is already a Flarm-Aircraft-Type
  switch (aircraftType)
  {
  case aircraft_t::paraglider:
...
int8_t FanetLora::getTrackingInfo(Frame *frm)
{
  ...
  index++;
  if (actTrackingData.heading < 0)
    actTrackingData.heading += 360.0;
  newData = true;
  return 0;
}
